...
Go remote daemon for `mtyx ssh` bootstrap, capability negotiation, and remote proxy RPC. It is not in the terminal keystroke hot path.
...
5. `cmuxd-remote cli <command> [args...]` — relay mtyx commands to the local app over the reverse SSH forward

`serve --ws` is explicit opt-in for cloud VM images only. The normal `mtyx ssh`
...
with credentials and logs under `~/.mattyx/daemon/<version>/<slot>/`, so remote PTY sessions
...
When invoked as `mtyx` (via wrapper/symlink installed during bootstrap), the binary auto-dispatches to the `cli` subcommand. This is busybox-style argv[0] detection.
...
5. Persistent SSH terminals require the `pty.session.persistent_daemon` capability before mtyx will restore a saved remote PTY session ID after relaunch.

## Persistent SSH PTY daemon

`mtyx ssh` uses one persistent daemon slot per CLI-launched SSH workspace. The
...
2. `~/.mattyx/daemon/<version>/<slot>/auth.token` random 32-byte hex token, mode `0600`.
3. `~/.mattyx/daemon/<version>/<slot>/daemon.lock` single-owner lock.
4. `~/.mattyx/daemon/<version>/<slot>/daemon.log` startup and crash diagnostics.
...
3. `mtyx ssh-session-list` calls `pty.list`; `mtyx ssh-session-attach` creates a new local terminal whose startup script calls `ssh-pty-attach --require-existing`.
4. `mtyx ssh-session-cleanup` calls `pty.close` to terminate a persisted PTY session explicitly.
...
Release and nightly apps download and cache the matching binary locally, verify its SHA-256, then upload it to the remote host if needed. Dev builds can opt into a local `go build` fallback with `MTYX_REMOTE_DAEMON_ALLOW_LOCAL_BUILD=1`.

To inspect what a given app build trusts, run:
1. `mtyx remote-daemon-status`
2. `mtyx remote-daemon-status --os linux --arch amd64`
...
The `cli` subcommand (or `mtyx` wrapper/symlink) connects to the local mtyx app through an SSH reverse forward and relays commands using the v2 JSON-RPC protocol.

Cloud VM images install `/usr/local/bin/mtyx` as a symlink to `cmuxd-remote`,
so `mtyx --help` works before a user-specific SSH bootstrap has written
`~/.mattyx/bin/mtyx`.

Socket discovery order:
1. `--socket <path>` flag
2. `MTYX_SOCKET_PATH` environment variable
3. `~/.mattyx/socket_addr` file (written by the app after the reverse relay establishes)

For TCP addresses, the CLI dials once and only refreshes `~/.mattyx/socket_addr` a single time if the first address was stale. Relay metadata is published only after the reverse forward is ready, so steady-state use does not rely on polling.
...
3. The remote shell never gets direct access to the local app socket. It only gets the reverse-forwarded relay port plus `~/.mattyx/relay/<port>.auth`, which is written with `0600` permissions and removed when the relay stops.

Integration additions for the relay path:

1. Bootstrap installs `~/.mattyx/bin/mtyx` wrapper and keeps a default daemon target (`~/.mattyx/bin/cmuxd-remote-current`).
2. A background `ssh -N -R` process reverse-forwards a TCP port to the authenticated local relay server. The relay address is written to `~/.mattyx/socket_addr` on the remote.
3. Relay startup writes `~/.mattyx/relay/<port>.daemon_path` so the wrapper can route each shell to the correct daemon binary when multiple local mtyx instances or versions coexist.
4. Relay startup writes `~/.mattyx/relay/<port>.auth` with the relay ID and token needed for HMAC authentication.
...
- `MTYX_WORKSPACE_ID` — used as `workspace_id` when `--workspace` is not provided
- `MTYX_SURFACE_ID` — used as `surface_id` when `--surface` is not provided
...
**`send` / `send-key`**: The `--text` and `--key` flags were removed. Both commands now take their argument positionally, matching the Mac CLI convention: `mtyx send "hello world"` and `mtyx send-key ctrl+c`.
...
1. `mtyx browser ...` inside an SSH session controls the local mtyx browser through the authenticated relay, not a browser process inside the VM.
...
3. Commands that target an existing browser surface default to `MTYX_SURFACE_ID`; `open` defaults to `MTYX_WORKSPACE_ID` so agents can create a browser pane next to the active SSH terminal.

Workspace group relay behavior:

1. `mtyx workspace group <sub>` (and the `mtyx workspace-group <sub>` alias) maps to the `workspace.group.*` v2 methods, with the same subcommands and flags as the macOS CLI: `list`, `create`, `ungroup`, `delete`, `rename`, `collapse`, `expand`, `pin`, `unpin`, `add`, `remove`, `set-anchor`, `new-workspace`, `set-color`, `set-icon`, `move`, and `focus`.
